...
A file like `.env.local` is often absent on some machines. From v3.0.1 Ahoy
reports that rather than passing over it in silence, so mark such entries
optional - see below.
...
[imports](/guides/importing/): the file is expected not to
always be there.
